function [bestfit,Bestcost1,Bestcost2,pop,tk]=MGKSO(fobj,fhd,lb,ub,dim,N,FF)
% ub=100*ones(1,dim);
% lb=-100*ones(1,dim)
%%初始化
% f=zeros(1,20);
...
%    alpha2=0.2*cos(t*pi*(-0.05))+0.2;
    ...
    for i=1:N
        for j=1:dim
            r1=rand;
            t1=(fit(i,1)-min(fit))/(min(fit)+eps);
...
%             new_X(i,j)=X(i,j)*abs(sin(r1))+r2*sin(r1)*((lb(:,j)+rand*(ub(:,j)-lb(:,j)))/(t))*levy(2)*abs(theta1*xbest(:,j)-theta2*X(i,j));
            new_X(i,j)=X(i,j)+(lb(:,j)+r1*(ub(:,j)-lb(:,j)))/t1;
...
%            new_fit(i,1) = feval(fhd,X(i,:)',fobj)-FF;
...
%            [~,idxxx]=find( new_fit<fit);
%             fit(idxxx)=new_fit(idxxx);
%             X(idxxx,:)=new_X(idxxx,:);
