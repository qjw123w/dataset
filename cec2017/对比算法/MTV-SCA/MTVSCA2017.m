...
function [bestfit,Bestcost1,Bestcost2,pop,t]=MTVSCA2017(fobj,fhd,lb,ub,dim,search_agent,FF)
...
%     X_fitness(i) = fobj(X(i,:));
    X_fitness(i) = feval(fhd,X(i,:)',fobj)-FF;
    FE=FE+1;
    if bestfit1>X_fitness(i)
        bestfit1=X_fitness(i);
    end    
...
while  FE < MaxFES
    ...
    if winTVPIdx == 1
        array_PoolTVP2 = permutation(1:round(lambda*N));
%         array_PoolTVP1 = permutation(lambda*N+1:2*lambda*N);
array_PoolTVP1 = permutation(round(lambda*N)+1:2*round(lambda*N));
        array_SCTVP = permutation(2*round(lambda*N)+1:end);
    elseif winTVPIdx == 2
        array_PoolTVP2 = permutation(1:round(lambda*N));
        array_SCTVP = permutation(round(lambda*N)+1:2*round(lambda*N));
        array_PoolTVP1  = permutation(2*round(lambda*N)+1:end);
    elseif winTVPIdx == 3
        array_SCTVP = permutation(1:round(lambda*N));
        array_PoolTVP1 = permutation(round(lambda*N)+1: 2*round(lambda*N));
        array_PoolTVP2  = permutation(2*round(lambda*N)+1:end);
    end
...
%         SCTVP_fit(i) = fobj(SCTVP_pop(i,:));%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        SCTVP_fit(i)=feval(fhd,SCTVP_pop(i,:)',fobj)-FF;
        if bestfit1>SCTVP_fit(i)
            bestfit1=SCTVP_fit(i);
        end
        FE=FE+1;
        Bestcost1(FE)=bestfit1;
    end
...
%         Pool_TVP_fit(i) = fobj(Pool_TVP(i,:));
         Pool_TVP_fit(i) =feval(fhd,Pool_TVP(i,:)',fobj)-FF;
         FE=FE+1;
         if bestfit1>Pool_TVP_fit(i)
            bestfit1=Pool_TVP_fit(i);
         end
        Bestcost1(FE)=bestfit1;
    end
...
%         mixVal_trial(i) = fobj(X_trial(i,:));
        mixVal_trial(i) =feval(fhd,X_trial(i,:)',fobj)-FF;
        FE=FE+1;
         if bestfit1>mixVal_trial(i)
            bestfit1=mixVal_trial(i);
         end
        Bestcost1(FE)=bestfit1;
