function [best_fit,curve,X]=IGKSO(N,Max_iter,lb,ub,dim,fobj)
% ub=100*ones(1,dim);
% lb=-100*ones(1,dim)
%%初始化
% f=zeros(1,20);
...
for t=1:Max_iter
    ...
    pheromone = (max(fit) - fit(i)) ./ (max(fit) - min(fit));
    
    % Threshold for identifying poor individuals
    pheromone_threshold = 0.35;
    
    % Find indices of poor individuals
    poor_idx = find(pheromone < pheromone_threshold);
    
    % Process each poor individual
...
%    alpha2=0.2*cos(t*pi*(-0.05))+0.2;
    ...
    alpha=abs(beta.*sin(3*pi/2+sin(3*pi*beta/2)));
        for i = 1:length(poor_idx)
        idx = poor_idx(i);
        
        % Select two random distinct solutions (different from current)
        r1 = randi(N);
        while r1 == idx
            r1 = randi(N);
        end
        ...
        % Generate new position (Eq. 22)
        b = randi([0, 1],1,dim); % Random binary value (0 or 1)
        Xnew(i,:) = Xm + 0.5 * (X(r1,:) - ((-1).^b) .* X(r2,:));
        ...
        fit_new(i) =feval(fobj,Xnew(i,:));
        
        % Greedy selection (Eq. 23)
        if fit_new(i) < fit(idx)
            X(idx,:) = Xnew(i,:);
           fit(idx) = fit_new(i);
        end
        end 
...
%              end
    ...
    for i=1:N
     ...
     for i = 1:N
%         if rand() < 0.5 % Eq. 24
            % Select three random distinct solutions
            idxs = randperm(N, 3);
            Xr = X(idxs(1),:);
            Xn = X(idxs(2),:);
            Xp = X(idxs(3),:);
            
            % Calculate average position (Eq. 25)
            Xavg = (Xr + Xn + Xp) / 3;
            
            % Calculate parameters
            c = 5 * rand(); % Eq. 26
            w = exp(-c * (t / Max_iter)) * rand() * 2; % Eq. 26
            betaj = rand(); % Random between 0 and 1
            r = randi([-1, 1]); % Random integer: -1, 0, or 1
            for j=1:dim
            % Calculate Xnew1 (Eq. 27)
            Xnew1(i,j) = (1 - betaj) * xbest(:,j) + betaj * Xavg(:,j) ;
             if rand() < 0.5 % Eq. 24
            % Calculate Xnew2 (Eq. 24)
            if w < 1
                Xnew2(i,j) =Xnew1(i,j) + r * w * abs(Xnew1(i,j) - Xavg(:,j)  + randn());
            else
                Xnew2(i,j) = (Xnew1(i,j) - Xavg(:,j) ) + r * w * abs(Xnew1(i,j) - Xavg(:,j)  + randn());
            end
             else
               Xnew2(i,j)= Xnew1(i,j);  
             end
            % Additional chance for improvement (Eq. 28)
            if rand() < w
                SF = rand();
                v = 2 * rand();
                Xrand(:,j)  = lb(:,j)  + rand*(ub(:,j) -lb(:,j) ); % Random solution
                Xmod(i,j)  = (Xnew2(i,j)  - rand()*Xnew2(i,j) ) + SF*(rand()*Xrand(:,j)  + v*xbest(:,j)  - Xnew2(i,j) );
                X_new(i,j) = Xmod(i,j) ;
            else
                X_new(i,j) = Xnew2(i,j) ;
            end
...
%         end
    ...
    X_qbest = QOBL(xbest, lb, ub);
    fit_q =feval(fobj, X_qbest);
    if fit_q < min(fit)
    xbest = X_qbest;
    best_fit=fit_q;
    end
