%这里面均值是先求500代中的一个最优值，再独立运行20次之后对这20个值求平均值
%源代码中均值是通过每一次函数调用求得的最优值，再独立运行20次之后对这20个值求平均值，不涉及代数问题。
...
%         [lb, ub, dim, fobj, grad_obj] = Get_Functions_cec2019_with_grad(F);
        [best_fit,GKSO_curve,X]=IGKSO(N,Max_iter,lb,ub,dim,fobj);
%         plot(S);
...
%         disp(['第F', num2str(F), '的最优值值为：', num2str(best)]);
        disp(['第F', num2str(F), '的biaozhuncha值为：', num2str(std1)]);
        for j=1:Max_iter/50
            p(1)=1;
            p(j+1)=j*50;
        end
