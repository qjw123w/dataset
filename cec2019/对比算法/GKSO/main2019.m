...
hanshu.pj=[];
hanshu.dd=[];
GKSO_huatu= repmat(hanshu, 10, 1);
for i =1:10
    F=i;
    for lun=1:runs
        [lb,ub,dim,fobj] = Get_Functions_cec2019(F);       
%         [best_fit,GKSO_curve,X]=cheng14(N,Max_iter,lb,ub,dim,fobj);
         [bestfit,Bestcost1,Bestcost2,pop,tk]=GKSO(fobj,lb,ub,dim,N);
         fit_1(lun)=bestfit;
        hanshupj(lun,:)=Bestcost1;
        hanshudd(lun,:)=Bestcost2;
        diedaihui_GKSO(i,:)=Bestcost1;
    end
    GKSO_huatu(i).pj=hanshupj;
    GKSO_huatu(i).dd=hanshudd;
