...
hanshu.pj=[];
hanshu.dd=[];
IGKSO2_huatu= repmat(hanshu, 10, 1);
for i =4:6:10
    F=i;
    for lun=1:runs
        [lb,ub,dim,fobj] = Get_Functions_cec2019(F);       
%         [best_fit,GKSO_curve,X]=cheng14(N,Max_iter,lb,ub,dim,fobj);
         [bestfit,Bestcost1,Bestcost2,pop,tk]=IGKSO20192(fobj,lb,ub,dim,N);
         fit_1(lun)=bestfit;
        hanshupj(lun,:)=Bestcost1;
        hanshudd(lun,:)=Bestcost2;
        diedaihui_IGKSO2(i,:)=Bestcost1;
    end
    IGKSO2_huatu(i).pj=hanshupj;
    IGKSO2_huatu(i).dd=hanshudd;
